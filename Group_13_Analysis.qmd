...
```{r}
#formula
formula1 <- as.formula(paste("Qualityclass_binary ~", paste(names(data1)[1:5], collapse = "+")))
...
$$ \ln\left(\frac{P({Qualityclass_binary}_i = 1)}{1 - P({Qualityclass_binary}_i = 1)}\right) = \alpha + \beta \cdot {characteristics}_i + \sum (\gamma_i \cdot {CountryVariables}_i) $$

where

1. ${Qualityclass_binary}_i$ is the response variable for the $i_th$ observation, indicating the quality classification of coffee.
...
4. ${characteristics}_i$ is the main characteristic derived from PCA analysis for the $i_th$ observation.

5. $\gamma_i$ is the impact of specific countries of origin on coffee quality.

6. ${CountryVariables}_i$ are dummy variables for the country of origin for the $i_th$ observation.

Finally, we can plot the probability of being Good as follows:

```{r}
data1 <- data1 %>%
         mutate(probs.Good = fitted(stepwise_model))
ggplot(data = data1, aes(x = characteristics, y = probs.Good)) +
     geom_smooth(method="glm", method.args = list(family="binomial"), se = FALSE) +
   labs(x = "characteristics", y = "Probability of coffee being Good")
```
